...
export default function Home() {
  ...
  const fetchKarya = useCallback(async (pageTarget: number, isInitial = false) => {
    try {
      ...
      setKaryaList((prev) => {
        const updatedList = isInitial ? newKarya : [...prev, ...newKarya];
        sessionStorage.setItem('hk_karya_list', JSON.stringify(updatedList));
        return updatedList;
      });

      if (newKarya.length < LIMIT) {
        setHasMore(false);
        sessionStorage.setItem('hk_has_more', 'false');
      } else {
        setHasMore(true);
        sessionStorage.setItem('hk_has_more', 'true');
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    const savedKarya = sessionStorage.getItem('hk_karya_list');
    const savedPage = sessionStorage.getItem('hk_page');
    const savedHasMore = sessionStorage.getItem('hk_has_more');
    const savedScrollY = sessionStorage.getItem('hk_scroll_y');
    const savedSearch = sessionStorage.getItem('hk_search');
    const savedKategori = sessionStorage.getItem('hk_kategori');

    if (savedSearch) setSearchQuery(savedSearch);
    if (savedKategori) setSelectedKategori(savedKategori);

    if (savedKarya && savedPage) {
      setKaryaList(JSON.parse(savedKarya));
  ...
  useEffect(() => {
    if (!isInitialRender.current) {
      sessionStorage.setItem('hk_page', page.toString());
    }
  }, [page]);

  useEffect(() => {
    sessionStorage.setItem('hk_search', searchQuery);
  }, [searchQuery]);

  useEffect(() => {
    sessionStorage.setItem('hk_kategori', selectedKategori);
  }, [selectedKategori]);
  ...
  useEffect(() => {
    const controlNavbar = () => {
      if (typeof window !== 'undefined') {
        ...
        sessionStorage.setItem('hk_scroll_y', currentScroll.toString());
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !isInitialRender.current) {
  ...
  const filteredKarya = karyaList.filter((karya) => {
    const matchesSearch =
      ...
      karya.isi.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900 pt-16">
      ...
      {/* FIXED HEADER */}
      <header
        className={`fixed top-0 left-0 right-0 h-16 bg-white/95 backdrop-blur-md border-b border-slate-200/80 z-50 transition-transform duration-300 flex items-center justify-between px-3 sm:px-12 ${
          showHeader ? 'translate-y-0' : '-translate-y-full'
        }`}
      >
        {/* Sisi Kiri: Brand Identity dengan Logo Utama */}
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          ...
          />
          {/* Di mobile tersembunyi (hidden), tampil mulai ukuran md */}
          <span className="hidden md:inline-block text-sm sm:text-base font-black uppercase bg-gradient-to-r from-indigo-600 to-indigo-800 bg-clip-text text-transparent tracking-tight whitespace-nowrap">
            Karya Tulis Perpustakaan Lubangsa
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 max-w-md w-full ml-2 justify-end" suppressHydrationWarning>
          <div className="relative w-full max-w-[130px] xs:max-w-[180px] sm:max-w-xs">
            <input
              ...
              className="w-full pl-3 pr-3 py-1.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition bg-slate-50 shadow-inner text-xs placeholder:text-slate-400"
            />
          </div>
          ...
          <div className="relative flex-shrink-0" ref={dropdownRef}>
            <button
              ...
              className="flex items-center justify-between gap-1.5 px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition min-w-[75px] sm:min-w-[100px]"
            >
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-12 py-6 sm:py-12">
        ...
        {/* Initial Skeleton Loader - Menyesuaikan bentuk grid 2 kolom */}
        {loading && (
          <div className="grid gap-3 sm:gap-8 grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 h-64 sm:h-80 border border-slate-200 animate-pulse flex flex-col justify-between">
                <div>
        ...
        {!loading && !error && (
          <>
            {filteredKarya.length === 0 ? (
              ...
              // grid-cols-2 memastikan di mobile selalu tampil 2 kolom berdampingan
              <div className="grid gap-3 sm:gap-8 grid-cols-2 lg:grid-cols-3 items-start">
                {filteredKarya.map((karya) => (
                  ...
                  >
                    <article className="bg-white rounded-xl sm:rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden hover:shadow-xl hover:border-slate-300/80 transition-all duration-300 flex flex-col h-full justify-between min-h-[340px] sm:min-h-[420px]">
                      <div className="flex flex-col flex-grow">
                        {karya.foto_url ? (
                          ...
                          <div className="h-2 sm:h-3 w-full bg-gradient-to-r from-indigo-500/20 via-purple-500/20 to-pink-500/20 flex-shrink-0" />
                        )}
                        ...
                        <div className="p-3 sm:p-6 flex flex-col flex-grow">
                          ...
                          <h2 className="text-sm sm:text-lg font-bold text-slate-900 mb-1 sm:mb-2 line-clamp-2 tracking-tight group-hover:text-indigo-600 transition-colors">
                            {karya.judul}
                          </h2>
                          <p className="text-slate-600 text-xs sm:text-sm line-clamp-3 sm:line-clamp-4 whitespace-pre-line leading-relaxed flex-grow">
                            {karya.isi}
                          </p>
                        </div>
                      </div>
                      
                      {/* FOOTER KARTU REKREASI DATA */}
